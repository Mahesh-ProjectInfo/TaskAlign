...
import { isValidRoleName } from "@/utils/validators.js";
import { MD_ASSIGNMENT_TYPES } from "./constants.js";

export default function RoleForm({ mode = "add", form, onChange, error, onSave, onCancel }) {
  ...
  const handleNameChange = (e) => {
    ...
    if (touched.name) {
      setFieldErrors((prev) => ({ ...prev, name: isValidRoleName(clean) || "" }));
    }
  };

  const handleNameBlur = () => {
    setTouched((prev) => ({ ...prev, name: true }));
    setFieldErrors((prev) => ({ ...prev, name: isValidRoleName(form.name) || "" }));
  };
  ...
  const handleSubmit = (e) => {
    ...
    const nameErr = isValidRoleName(form.name) || "";
